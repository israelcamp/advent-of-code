...
function calculate_hand_strength(hand)
	...
	local max_score = 7
	if joker_count == #hand then
		return max_score
	end

	-- getting the maximum count and character
	local maxs = get_max(char_count)
	local max_count = maxs[1]
	local max_char = maxs[2]

	char_count[max_char] = max_count + joker_count
...
local card_strengths = { "A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J" }

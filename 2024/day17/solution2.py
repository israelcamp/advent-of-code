...
if __name__ == "__main__":
    ...
    trimmed_program = program[:-2]  # removes 0,3

    range_offset = range(0, 2**3)  # it will always be the opcode 2 and the operand 3
    A = 0

    target_output = program[-2]
    current_program_outputs = solution_part_one(
        Register(A=A, B=0, C=0), trimmed_program
    )
    print("START A", A)
    for idx, target_output in enumerate(reversed(program)):
        A *= 8
        reg = Register(A=A, B=0, C=0)
        print(target_output, idx, program[-idx - 1 :], A)
        offset = 0
